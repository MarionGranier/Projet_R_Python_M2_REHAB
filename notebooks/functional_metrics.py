...
def test_functional(alphas, treshold_symregion_degrees=30, treshold_amp_degrees=30):
    """ 
    alphas : ndarray with an epoch (window) on each row  
    treshold_symregion_degrees : the threshold for the symmetric region in degrees
    treshold_amp_degrees : the threshold for the amplitude in degrees

    return: true if there is a functional movement is the epoch (row) 
    """
